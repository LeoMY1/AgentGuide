...
grep -q "uses: actions/checkout@v7" "$WORKFLOW_FILE"
grep -q "uses: actions/setup-node@v7" "$WORKFLOW_FILE"
grep -q "node-version: '22'" "$WORKFLOW_FILE"
grep -q "uses: actions/upload-pages-artifact@v5" "$WORKFLOW_FILE"
grep -q "uses: actions/configure-pages@v6" "$WORKFLOW_FILE"
grep -q "uses: actions/deploy-pages@v5" "$WORKFLOW_FILE"
